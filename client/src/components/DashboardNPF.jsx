...
const DashboardNPF = ({ user, dashboardData }) => {
    const [npfLoading, setNpfLoading] = useState(false);
    const [selectedPeriod, setSelectedPeriod] = useState('');
    const [npfData, setNpfData] = useState({
        historical: []
    });
        ...
        const loadAllData = async () => {
            try {
                ...
                if (npfResponse.success) {
                    processNpfData(npfResponse.data);
                } else {
                    setError(npfResponse.error);
                }
    ...
    // Process NPF data
    const processNpfData = (dataFromBackend) => {
        // Sort by date (period)
    ...
    // Hitung persentase NPF vs PBY (NPF Ratio)
    const calculateNPFRatio = (npfValue, pbyValue) => {
        if (!pbyValue || pbyValue === 0) return 0;
        return (npfValue / pbyValue) * 100;
    };

    // Update data ketika selectedPeriod berubah
    useEffect(() => {
        if (npfData.historical.length > 0) {
    ...
    // Hitung MTD
    const calculateMTD = () => {
        if (npfData.historical.length < 2) return {
            npf: 0, cfg: 0, pwg: 0,
            griya: 0, oto: 0, mitraguna: 0, pensiun: 0, cicil_emas: 0
        };
        
        const currentIndex = npfData.historical.findIndex(item => item.period === selectedPeriod);
        if (currentIndex <= 0) return {
            npf: 0, cfg: 0, pwg: 0,
            griya: 0, oto: 0, mitraguna: 0, pensiun: 0, cicil_emas: 0
        };
        
        const currentData = npfData.historical[currentIndex];
        const previousData = npfData.historical[currentIndex - 1];
        
        return {
            npf: currentData.npf - previousData.npf,
    ...
    // Hitung YTD
    const calculateYTD = () => {
        if (npfData.historical.length === 0) return {
            npf: 0, cfg: 0, pwg: 0,
            griya: 0, oto: 0, mitraguna: 0, pensiun: 0, cicil_emas: 0
        };
        
        const currentData = npfData.historical.find(item => item.period === selectedPeriod);
        const oldestData = npfData.historical[0];
        ...
        return {
            npf: currentData.npf - oldestData.npf,
    ...
    // Get performance cards - URUTAN: NPF → CFG → Griya → Oto → Mitraguna → Pensiun → PWG → Cicil Emas
    const getPerformanceCards = () => {
        const latestData = npfData.historical.find(item => item.period === selectedPeriod);
        ...
        return [
            {
                ...
                mtd: formatForPerformanceCard(mtdData.npf),
                ytd: formatForPerformanceCard(ytdData.npf),
                mtdTrend: mtdData.npf >= 0 ? 'up' : 'down',
                ytdTrend: ytdData.npf >= 0 ? 'up' : 'down',
    ...
    return (
        <>
                    ...
                    <div className="flex items-center justify-between mb-6">
                        ...
                        <div className="flex items-center space-x-2">
                            ...
                            <button
                                ...
                                className="inline-flex items-center px-3 py-1.5 border border-gray-300 rounded-lg text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500"
                                disabled={npfLoading}
                            >
                                <RefreshCw className={`w-4 h-4 ${npfLoading ? 'animate-spin' : ''}`} />
                            </button>
                        </div>
                    </div>
                    <div className="flex flex-wrap gap-3">
                        {npfData.historical.map((item) => (
                            <button
                                key={item.period}
            ...
            {/* Performance Cards - URUTAN: NPF → CFG → Griya → Oto → Mitraguna → Pensiun → PWG → Cicil Emas */}
            <motion.div
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
            ...
            >
                <h2 className="text-xl font-bold text-gray-900 mb-6 flex items-center">
                    <LineChart className="w-6 h-6 mr-3 text-red-500" />
                    Kinerja NPF {selectedPeriod}
                </h2>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    ...
                    {/* Pie Chart - Komposisi NPF per Segment */}
                        ...
                        <div className="p-6">
                            ...
                            <div className="flex flex-col items-center">
                                <div className="relative w-32 h-32 mb-4">
                                    {(() => {
                                        const periodData = npfData.historical.find(hist => hist.period === selectedPeriod);
                                        
                                        if (periodData) {
                                            const npfValue = periodData.npf || 0;
                                            
                                            // Jika npfValue = 0, tampilkan lingkaran kosong dengan angka 0
                                            if (npfValue === 0) {
                                                return (
                                                    <div className="w-full h-full rounded-full bg-gray-100 flex items-center justify-center">
                                                        <div className="text-center">
                                                            <p className="text-lg font-bold text-gray-900">0</p>
                                                            <p className="text-xs text-gray-600">Total NPF</p>
                                                        </div>
                                                    </div>
                                                );
                                            }
                                            ...
                                            const percentages = segments.map(seg => ({
                                                ...seg,
                                                percent: (seg.value / npfValue) * 100
                                            }));
                                                    ...
                                                    <div className="absolute inset-0 flex items-center justify-center">
                                                        <div className="text-center">
                                                            <p className="text-lg font-bold text-gray-900">
                                                                {formatForPerformanceCard(npfValue)}
                                                            </p>
                                                            <p className="text-xs text-gray-600">Total NPF</p>
                                                        </div>
                                                    </div>
                                ...
                                <div className="w-full">
                                    <div className="space-y-2">
                                        {(() => {
                                            ...
                                            // Jika npfValue = 0, tampilkan semua segment dengan nilai 0
                                            const segments = [
                                                { 
                                                    name: 'Griya', 
                                                ...
                                                <div key={index} className="flex items-center justify-between p-2 bg-gray-50 rounded-lg">
                                                    ...
                                                    <div className="text-right">
                                                        ...
                                                        <p className="text-xs text-gray-500">
                                                            {npfValue !== 0 ? ((item.value / npfValue) * 100).toFixed(1) : '0.0'}%
                                                        </p>
                                                    </div>
                                                </div>
                                            ));
            ...
            {/* Tabel Data Historis - URUTAN: NPF → CFG → Griya → Oto → Mitraguna → Pensiun → PWG → Cicil Emas */}
            <motion.div
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                ...
                <div className="bg-white rounded-2xl shadow-lg overflow-hidden">
                    <div className="px-6 py-4 border-b border-gray-200 bg-gradient-to-r from-gray-50 to-gray-100">
                        <div className="flex justify-between items-center">
                            <h2 className="text-xl font-bold text-gray-900 flex items-center">
                                <AlertTriangle className="w-6 h-6 mr-3 text-red-500" />
                                Data Historis NPF {selectedPeriod}
                            </h2>
                            <div className="text-sm text-gray-500">
                                {npfData.historical.length} periode data
                            </div>
                        </div>
                    </div>
                        ...
                        <table className="min-w-full divide-y divide-gray-200">
                            <thead className="bg-gray-50">
                                <tr>
                                    ...
                                    {npfData.historical.map((item, index) => (
                            ...
                            <tbody className="divide-y divide-gray-200">
                                {[
                                    ...
                                    const currentData = npfData.historical[currentIndex];
                                    const firstData = npfData.historical[0];
                                    const previousData = currentIndex > 0 ? npfData.historical[currentIndex - 1] : null;
                                    
                                    // Helper function to get value based on label
                                    const getValue = (label, data) => {
                                        switch(label) {
                                            case 'NPF': return data.npf || 0;
                                ...
                                {/* Row % NPF Ratio - Auto Calculated: NPF / PBY * 100% */}
                                <tr className="hover:bg-gray-50 bg-gray-50/50">
                                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                        % NPF Retail
                                    </td>
                                    {npfData.historical.map((npfItem, index) => {
                                        const pbyItem = getPbyDataByPeriod(npfItem.period);
                                        const npfValue = npfItem.npf || 0;
                                        const pbyValue = pbyItem?.pby || 0;
                                        const percentage = calculateNPFRatio(npfValue, pbyValue);
                                ...
                                {/* Row % CFG Ratio - Auto Calculated: CFG(NPF) / CFG(PBY) * 100% */}
                                <tr className="hover:bg-gray-50 bg-gray-50/50">
                                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                        % CFG
                                    </td>
                                    {npfData.historical.map((npfItem, index) => {
                                        const pbyItem = getPbyDataByPeriod(npfItem.period);
                                        const cfgNpfValue = npfItem.cfg || 0;
                                        const cfgPbyValue = pbyItem?.cfg || 0;
                                        const percentage = calculateNPFRatio(cfgNpfValue, cfgPbyValue);
                                ...
                                {/* Row % PWG Ratio - Auto Calculated: PWG(NPF) / PWG(PBY) * 100% */}
                                <tr className="hover:bg-gray-50 bg-gray-50/50">
                                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                        % PWG
                                    </td>
                                    {npfData.historical.map((npfItem, index) => {
                                        const pbyItem = getPbyDataByPeriod(npfItem.period);
                                        const pwgNpfValue = npfItem.pwg || 0;
                                        const pwgPbyValue = pbyItem?.pwg || 0;
                                        const percentage = calculateNPFRatio(pwgNpfValue, pwgPbyValue);
